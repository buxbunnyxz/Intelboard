        ...
        <div class="card-body">
            ...
            <form id="calculation-form" class="mt-4" style="display:none;">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" min="0" max="100" class="form-control" id="percentage"
                        required placeholder="{{ __('messages.enter_broker_percentage') }}">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" min="0" class="form-control" id="cash-advance"
                        value="0">
                </div>
            ...
            <div id="result-alert" style="display:none;">
                <div class="alert alert-primary alert-dismissible bg-primary text-white border-0 fade show mt-4"
                    role="alert">
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                        aria-label="Close"></button>
                    <strong>
                        {{ __('messages.final_amount_save_prompt') }} <span id="final-amount"></span>,
                        {{ __('messages.wannasavit') }}
                    </strong>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (startBtn) {
                startBtn.addEventListener('click', function(e) {
                    ...
                    setTimeout(function() {
                        vehiculeRental = parseFloat(document.getElementById('vehicule-rental-price')
                            .value) || 0;
                        percentage = parseFloat(document.getElementById('percentage').value) || 0;
                        bonus = parseFloat(document.getElementById('bonus').value) || 0;
                        cashAdvance = parseFloat(document.getElementById('cash-advance').value) ||
                            0;
                    ...
                    try {
                        const res = await fetch('{{ route('calculate.save') }}', {
                            ...
                            body: JSON.stringify({
                                driver_id: {{ $driver->id }},
                                week: "{{ $week }}",
                                vehicule_rental_price: parseFloat(document
                                    .getElementById('vehicule-rental-price').value
                                ) || 0,
                                broker_percentage: parseFloat(document.getElementById(
                                    'percentage').value) || 0,
                                bonus: parseFloat(document.getElementById('bonus')
                                    .value) || 0,
                                cash_advance: parseFloat(document.getElementById(
                                    'cash-advance').value) || 0
                            })
                        });
