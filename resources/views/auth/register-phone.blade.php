                ...
                <div class="col-xxl-4 col-lg-5">
                    <div class="card">
                        ...
                        <div class="card-body p-4">
                            ...
                            <!-- Main phone submission form (isolated so we can place buttons side-by-side outside it) -->
                            <form id="phone-form" action="{{ route('register.phone.store') }}" method="POST"
                                class="mb-0">
                                @csrf
                                <div class="mb-3">
                                    ...
                                    <input class="form-control" type="tel" id="phone_number" name="phone_number"
                                        required value="{{ old('phone_number') }}" maxlength="10" inputmode="numeric"
                                        placeholder="{{ __('messages.register_phone_placeholder') }}">
                                    <div class="invalid-feedback">
                                        {{ __('messages.register_phone_error') }}
                                    </div>
                                </div>
                            </form>

                            <!-- Buttons row: Cancel (left), Confirm (right) -->
                            <div class="row g-2 mt-2">
                                <div class="col-6">
                                    <form action="{{ route('register.phone.cancel') }}" method="POST"
                                        onsubmit="return confirm('{{ __('messages.register_phone_cancel_confirm') }}');">
                                        @csrf
                                        <button type="submit" class="btn btn-danger w-100">
                                            {{ __('messages.register_phone_cancel_button') }}
                                        </button>
                                    </form>
                                </div>
                                <div class="col-6">
                                    <!-- This button submits the #phone-form via HTML5 'form' attribute -->
                                    <button type="submit" form="phone-form" class="btn btn-primary w-100">
                                        {{ __('messages.register_phone_button') }}
                                    </button>
                                </div>
                            </div>
